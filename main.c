...
void start() {
    ...
    FILE *fptr;
     if ((fptr=fopen("data.txt","r+") ) == NULL)puts("File could not be opened");
     else
     {
         ...
         while(!feof(fptr))
         {
            struct studentNode* newNode = malloc(sizeof(struct studentNode));
               newNode->next=NULL;
        fscanf(fptr," %d %s %d %d %d %d %d %d %19[^\n]%*1[\n]",&newNode->BN,newNode->id,&newNode->studentgrades.math,&newNode->studentgrades.electronics,&newNode->studentgrades.circuits,&newNode->studentgrades.programming,&newNode->studentgrades.lang,&newNode->studentgrades.totalMarks,newNode->fullName);
if(feof(fptr))break;
        if(head==NULL)
                {
                    head=newNode;
...
void quit() {
    FILE *fptr;
     if ((fptr=fopen("data.txt","w+") ) == NULL)puts("File could not be opened");
    else
     {
       struct studentNode* current=head;
         ...
         while(current!=NULL )
         {
                printf("%d \t %15s \t %d \t %d \t %d \t %d \t %d \t %d \t %s \n",current->BN,current->id,current->studentgrades.math,current->studentgrades.electronics,current->studentgrades.circuits,current->studentgrades.programming,current->studentgrades.lang,current->studentgrades.totalMarks,current->fullName);
               fprintf(fptr," %d \t %15s \t %d \t %d \t %d \t %d \t %d \t %d \t %s \n",current->BN,current->id,current->studentgrades.math,current->studentgrades.electronics,current->studentgrades.circuits,current->studentgrades.programming,current->studentgrades.lang,current->studentgrades.totalMarks,current->fullName);
                current=current->next;
            }
